import functools
from base.rbtree_helper import show_rb_tree, save_rb_tree
import random

"""
红黑树当时没讲明白, 关于究竟是只能右为红还是左右都可以, 其实主要看理解和旋转方式
单侧红, 是方便更好理解和控制逻辑
这里做双侧都可以红的方式
"""
class RBNode:
...
class RBTree:
    ...
    def check_unlink_node(self, node):
        '''
        检查断开连接节点node
        :param node:
        :return:
        ...
        if brother.is_red_node():
            ...
            print("check node unlink more ")
            # 再重新检查当前节点
            self.check_unlink_node(node)
            return
        ...
        if all_none or all_black:
            ...
            if node.parent.is_red_node():
                ...
                return
            self.check_unlink_node(node.parent)
            return
    ...
    def unlink_node(self, val):

        node = self.get_node(val)
        if not node:
            print("node error {}".format(val))
            return
        save_rb_tree(self.root, "{}_unlink_0".format(val))
        # 获取真正要断开连接的节点
        node = self.pre_unlink_node(node)
        save_rb_tree(self.root, "{}_unlink_1".format(val))
        # node 节点必不为空，且子节点也都为空
        self.check_unlink_node(node)
        save_rb_tree(self.root, "{}_unlink_2".format(val))
        # 真正断开连接要断开连接的节点
        self.real_unlink_node(node)
        save_rb_tree(self.root, "{}_unlink_3".format(val))
        pass

    def real_unlink_node(self, node):
        '''
        真正断开连接节点函数
        :param node:
        :return:
